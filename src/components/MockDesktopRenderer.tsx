      ...
      <div className="flex-1 bg-neutral-900 p-6 text-white overflow-auto relative">
        <div className="h-full flex flex-col items-center justify-center text-neutral-500 gap-4">
          ...
          <p className="text-sm max-w-md text-center">
            This is a simulated application window running inside the Cloud 107 environment. In a
            real environment, this would stream the native application pixel-perfectly.
          </p>
        </div>
      </div>
    </motion.div>
    ...
    >
      ...
      <div className="absolute top-20 left-4 flex flex-col gap-4 z-0">
        ...
        >
          <Folder
            size={36}
            className="text-blue-400 drop-shadow-md group-hover:scale-105 transition-transform"
          />
          <span className="text-white text-xs font-medium text-center drop-shadow-md truncate w-full select-none bg-black/20 px-1 rounded">
            Projects
          </span>
        </div>
      </div>
        ...
        {contextMenu && (
          ...
          >
            <button
              ...
              className="w-full text-left px-4 py-2 text-sm text-neutral-200 hover:bg-white/10 hover:text-white flex items-center gap-3 transition-colors"
            >
      ...
      {/* Mac-style Dock */}
      <Dock apps={APPS} windows={windows} activeWindowId={activeWindowId} onOpenApp={openApp} />
    </div>
  );
}
