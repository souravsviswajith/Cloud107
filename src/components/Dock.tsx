        ...
        {apps.map((app, idx) => {
          ...
          return (
            <motion.button
              ...
              style={{ width: 48 }}
            >
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center bg-gradient-to-br from-white/20 to-white/5 border border-white/20 shadow-lg ${app.color}`}>
              ...
              <div className="absolute -bottom-1.5 w-full flex justify-center">
                <div className={`h-1 rounded-full transition-all duration-300 ${isOpen ? (isActive ? 'w-2 bg-white' : 'w-1 bg-white/50') : 'w-0'}`} />
              </div>
            </motion.button>
          )
        })}
