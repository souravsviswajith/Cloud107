...
import { Folder, Globe, FileText, Terminal, Settings, Wifi, Battery, Volume2, Activity, Gauge, Monitor, Network, WifiOff, Cpu } from 'lucide-react';
...
export function Desktop() {
  ...
  const [qualityProfile, setQualityProfile] = useState<'performance' | 'balanced' | 'quality'>('balanced');
  const [metrics, setMetrics] = useState({ fps: 60, bitrate: 8.5, latency: 12, resolution: { width: 1920, height: 1080 }, packetLoss: 0.01, codec: 'H.264 (NVENC)' });
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-neutral-900 to-[#050505] bg-fixed flex flex-col font-sans relative selection:bg-blue-500/30 selection:text-white">
      ...
      <div className="h-7 bg-neutral-900 border-b border-white/5 flex items-center justify-between px-4 text-[12px] text-neutral-400 select-none z-[100] shadow-sm relative">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-neutral-200 font-medium">{dateString} {timeString}</span>
        </div>
      </div>
          ...
          >
            ...
            <div className="p-4 space-y-4">
              <div className="space-y-2">
                <div className="text-neutral-400 uppercase tracking-wider text-[10px]">Quality Profile</div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-neutral-400 flex items-center gap-1.5"><Monitor size={12} /> Resolution</span>
                  <span className="text-white font-medium">{metrics.resolution.width}x{metrics.resolution.height}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-neutral-400 flex items-center gap-1.5"><Activity size={12} /> Framerate</span>
                  <span className={`font-medium ${metrics.fps < 45 ? 'text-amber-400' : 'text-emerald-400'}`}>{Math.round(metrics.fps)} FPS</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-neutral-400 flex items-center gap-1.5"><Network size={12} /> Bitrate</span>
                  <span className="text-blue-400 font-medium">{metrics.bitrate.toFixed(1)} Mbps</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-neutral-400 flex items-center gap-1.5"><Gauge size={12} /> Latency</span>
                  <span className={`font-medium ${metrics.latency > 30 ? 'text-amber-400' : 'text-emerald-400'}`}>{Math.round(metrics.latency)} ms</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-neutral-400 flex items-center gap-1.5"><WifiOff size={12} /> Packet Loss</span>
                  <span className="text-white font-medium">{metrics.packetLoss}%</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-neutral-400 flex items-center gap-1.5"><Cpu size={12} /> Codec</span>
          ...
          {showNotification && (
            ...
            >
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="text-xs text-neutral-400 leading-relaxed">Applications below are running remotely.</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 h-16 bg-neutral-900/90 backdrop-blur-xl border border-white/10 rounded-2xl px-3 flex items-center gap-3 shadow-2xl z-[100]">
        {[
          { id: 'files' as AppId, icon: Folder, color: 'text-blue-400', label: 'Files', disabled: true },
          { id: 'browser' as AppId, icon: Globe, color: 'text-emerald-400', label: 'Browser', disabled: false },
          { id: 'notes' as AppId, icon: FileText, color: 'text-amber-400', label: 'Notes', disabled: false },
          { id: 'terminal' as AppId, icon: Terminal, color: 'text-purple-400', label: 'Terminal', disabled: false },
          { id: 'settings' as AppId, icon: Settings, color: 'text-neutral-400', label: 'Settings', disabled: true },
        ].map((item, i) => {
