...
export function StreamingDesktopRenderer() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setMetrics(prev => {
        ...
        return {
          ...prev,
          fps: Math.max(30, Math.min(60, Math.round(targetFps * (1 + (Math.random() * 0.05 - 0.02))))),
          bitrate: Number(Math.max(1, targetBitrate * (1 + jitter)).toFixed(1)),
          latency: Math.max(2, Math.round(targetLatency * (1 + jitter * 2))),
          packetLoss: Number(Math.max(0, prev.packetLoss + (Math.random() * 0.02 - 0.01)).toFixed(2))
        };
      });
    }, 1000);
  ...
  return (
    <div className="flex-1 relative bg-neutral-950 overflow-hidden flex items-center justify-center" ref={containerRef}>
      {/* Settings toggle */}
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`font-medium ${metrics.fps < 45 ? 'text-amber-400' : 'text-emerald-400'}`}>{metrics.fps} FPS</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`font-medium ${metrics.latency > 30 ? 'text-amber-400' : 'text-emerald-400'}`}>{metrics.latency} ms</span>
                </div>
                <div className="flex items-center justify-between">
           ...
           <div className="absolute inset-0 bg-neutral-950">
             ...
             <div className="absolute inset-0" style={{ 
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none opacity-30">
                <Monitor size={64} className={`mb-4 transition-colors ${isFocused ? 'text-blue-500/20' : 'text-neutral-500/20'}`} />
                <div className={`font-mono text-sm transition-colors ${isFocused ? 'text-blue-500/40' : 'text-neutral-500/40'}`}>
          ...
          {status === 'reconnecting' && (
            <div className="flex flex-col items-center gap-4 bg-black/60 p-8 rounded-2xl backdrop-blur-md border border-white/10">
              ...
              <div className="text-center">
                <div className="text-amber-400 font-medium mb-1 text-lg">Connection Lost - Reconnecting</div>
          ...
          {status === 'failed' && (
            <div className="flex flex-col items-center gap-4 bg-black/60 p-8 rounded-2xl backdrop-blur-md border border-white/10">
              ...
              <div className="text-center mb-2">
                ...
                <div className="text-neutral-400 text-sm">Could not re-establish connection to the workspace.</div>
              </div>
              <button 
