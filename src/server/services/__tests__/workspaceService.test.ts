...
describe('WorkspaceService', () => {
  ...
  it('should list workspaces for a user', async () => {
    const mockWorkspaces = [{ id: '1', name: 'WS1', userId: 1, state: WorkspaceState.Offline, createdAt: '', updatedAt: '' }];
  ...
  it('should create a workspace', async () => {
    const mockWorkspace = { id: 'uuid-1', name: 'New WS', userId: 1, state: WorkspaceState.Offline, createdAt: '', updatedAt: '' };
  ...
  it('should start a workspace', async () => {
    const mockWorkspace = { id: '1', name: 'WS1', userId: 1, state: WorkspaceState.Starting, createdAt: '', updatedAt: '' };
  ...
  it('should throw if starting an unauthorized or non-existent workspace', async () => {
    ...
    await expect(workspaceService.startWorkspace('1', 1)).rejects.toThrow('Workspace not found or unauthorized');
  });
});
